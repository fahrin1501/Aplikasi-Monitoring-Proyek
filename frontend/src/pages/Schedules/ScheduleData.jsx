...
import api from '../../api';
import { 
  ArrowLeft, CalendarDays, Save, Loader2, AlertTriangle, Edit3, X, Filter, ListPlus, Clock, Target, CheckCircle2, Trash2, ChevronDown, Plus, Calendar 
} from 'lucide-react';
...
export default function ScheduleData() {
  ...
  const grandTotalRAB = scheduleData ? scheduleData.rab_data.reduce((sum, cat) => sum + cat.items.reduce((itemSum, item) => itemSum + Number(item.total_harga || 0), 0), 0) : 0;
  ...
  const handleInlineChange = (rabItemId, weekNum, value, bobotRAB) => {
    ...
    const realisasiAktual = scheduleData.realizations?.filter(r => r.rab_item_id === rabItemId)?.reduce((sum, r) => sum + parseFloat(r.bobot_realisasi), 0) || 0;
  ...
  const executeDeleteWeek = () => {
    ...
    setScheduleInputs(prev => {
      const newInputs = { ...prev };
      Object.keys(newInputs).forEach(key => { if (key.endsWith(`_${wNum}`)) delete newInputs[key]; });
      return newInputs;
    });
    setScheduleData(prev => ({
      ...prev,
    ...
    try {
      ...
      Object.keys(scheduleInputs).forEach(key => {
        ...
        if (value > 0) {
          ...
          // Mengambil informasi original untuk mempertahankan bulan & tanggal saat update parsial
          const originalSched = scheduleData.schedules.find(s => s.rab_item_id === parseInt(rab_item_id) && s.minggu_ke === parseInt(minggu_ke));

          payloadArr.push({
            rab_item_id: parseInt(rab_item_id),
            minggu_ke: parseInt(minggu_ke),
            bulan: originalSched?.bulan || null,
            tanggal_awal: originalSched?.tanggal_awal || null,
            tanggal_akhir: originalSched?.tanggal_akhir || null,
            bobot_rencana: value
          });
        }
      });
  ...
  // Di update ini kita gabungkan data dari backend dengan header Bulan dan Tanggal
  let weekGroups = {};
            ...
            <div className="flex items-center bg-white dark:bg-slate-800/80 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700/60 shadow-sm transition-all">
              {!isEditMode && (
                <button disabled={isLoading} onClick={() => navigate('/schedules/input')} className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[11px] font-bold rounded-lg transition-colors border border-emerald-200 dark:border-emerald-500/30 disabled:opacity-50 disabled:cursor-not-allowed">
                  <ListPlus className="w-3.5 h-3.5" /> Buat Rencana Baru
                </button>
              )}
                ...
                <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-700/60 bg-blue-50/50 dark:bg-blue-900/10 flex flex-col md:flex-row md:items-center justify-between gap-3 pr-12">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {isEditMode && canCreateData && (
                      ...
                      >
                        <Trash2 className="w-3.5 h-3.5" /> Kosongkan M-{weekNum}
                      </button>
                    )}
                  </div>
                </div>
