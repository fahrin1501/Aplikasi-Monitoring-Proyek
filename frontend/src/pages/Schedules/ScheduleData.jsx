...
export default function AddSchedule() {
  ...
  // Tangkap ID Proyek jika di-routing dari ScheduleData
  const passedProjectId = location.state?.projectId || '';

  // --- STATE DATA PROYEK ---
  const [projects, setProjects] = useState([]);
  const [selectedProjectId, setSelectedProjectId] = useState(passedProjectId);
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        // Auto fetch jika ada ID yang dilempar dari router
        if (passedProjectId) {
          fetchTimeSchedule(passedProjectId);
        }
      } catch (error) {
    ...
    <div className="w-full space-y-6 pb-24 relative animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-start gap-3">
          <button onClick={() => navigate(passedProjectId ? `/schedules/${passedProjectId}` : '/schedules')} className="p-2.5 bg-white dark:bg-slate-800 hover:bg-slate-100 border border-slate-200 dark:border-slate-700/80 text-slate-600 dark:text-slate-300 rounded-xl shadow-sm transition-colors"><ArrowLeft className="w-5 h-5" /></button>
          <div>
