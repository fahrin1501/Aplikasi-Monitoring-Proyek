...
import { 
  ...
  ChevronDown, Calendar, Layers, Plus, Trash2, CheckCircle2, ListPlus, AlertTriangle, Building2
} from 'lucide-react';

export default function AddSchedule() {
  const navigate = useNavigate();
  // PERBAIKAN: Mengambil ID langsung dari Parameter URL (/schedules/:id/data/input)
  const { id } = useParams();
  const projectId = id || '';
  ...
  // Fetch Daftar Proyek & Auto Fetch Jadwal
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        // Auto fetch karena ID sudah didapat dari URL
        if (projectId) {
          fetchTimeSchedule(projectId);
        }
      } catch (error) {
  ...
  const fetchTimeSchedule = async (pId) => {
    setIsLoadingSchedule(true);
    try {
      const res = await api.get(`/projects/${pId}/schedules`);
  ...
  const handleProjectSelect = (e) => {
    ...
    setDraftDivisiId('');
    setDraftItemId('');
    // Memaksa URL berubah agar tetap konsisten
    navigate(`/schedules/${pId}/data/input`);
    fetchTimeSchedule(pId);
  };
  ...
  // --- HANDLER KEMBALI (Mengikuti URL :id/data) ---
  const handleGoBack = () => {
    if (selectedProjectId) {
      navigate(`/schedules/${selectedProjectId}/data`);
    } else {
      navigate('/projects');
    }
  };
  ...
  // Render Peringatan Jika Akses Tanpa Pilih Proyek
  if (!selectedProjectId && !isLoadingProjects) {
    return (
      <div className="w-full space-y-6 pb-24 relative animate-fade-in">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            <button onClick={handleGoBack} className="p-2.5 bg-white dark:bg-slate-800 hover:bg-slate-100 border border-slate-200 dark:border-slate-700/80 text-slate-600 dark:text-slate-300 rounded-xl shadow-sm transition-colors"><ArrowLeft className="w-5 h-5" /></button>
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Silakan pilih proyek terlebih dahulu untuk mulai membuat jadwal.</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center min-h-[30vh] text-center px-4">
          ...
          <h2 className="text-lg font-bold text-slate-500 dark:text-slate-400 mb-2">Pilih Proyek Terlebih Dahulu</h2>
          <p className="text-xs text-slate-400">Data form dan keranjang pekerjaan akan muncul setelah Anda memilih proyek di atas.</p>
        </div>
      </div>
    );
  }
