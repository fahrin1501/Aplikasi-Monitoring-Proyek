...
import { 
  ...
  RotateCw, UserCheck, CloudSun, ListTodo, Users, 
...
export default function LaporanList() {
  ...
  const fetchLaporan = async (showMainLoader = true) => {
  ...
  const handleRefresh = () => fetchLaporan(false);
  ...
  // --- FUNGSI PEWARNAAN STATUS KONSISTEN (DARK TRANSPARENT) ---
  ...
  // Sembunyikan filter 'Ditolak' untuk Tamu
  const statusOptions = [
    ...
    <div className="space-y-6 w-full relative pb-20">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap lg:flex-nowrap items-center gap-2 sm:gap-3 w-full xl:w-auto">
          ...
          <div className="relative flex-1 lg:flex-none min-w-[140px] shadow-sm">
            ...
            <input type="text" placeholder="Cari laporan..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full lg:w-56 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 text-xs text-slate-800 dark:text-white pl-9 pr-4 py-2.5 md:py-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 transition-colors" />
          </div>

          {/* Filter Dropdown Pop-up */}
          <div className="relative" ref={filterRef}>
            <button onClick={() => setShowFilter(!showFilter)} className={`flex items-center gap-2 p-2.5 md:px-3.5 md:py-2 rounded-xl text-xs font-bold border transition-all shadow-sm ${showFilter || filters.status !== 'Semua' || filters.startDate || filters.endDate ? 'bg-amber-50 dark:bg-amber-500/10 border-amber-300 dark:border-amber-500/30 text-amber-600 dark:text-amber-500' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700/80 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
          ...
          {canCreateData && (
            <div className="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0">
              ...
              <button onClick={() => navigate('/laporan/input')} disabled={isEditMode} className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-amber-500 hover:bg-amber-600 text-white dark:text-slate-950 font-bold text-xs px-3.5 py-2.5 md:py-2 rounded-xl transition-all shadow-md active:scale-95 whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl shadow-sm">
          ...
          <p className="text-sm font-medium text-slate-600 dark:text-slate-300">Memuat data laporan...</p>
        </div>
      ) : (
        <>
          {/* ========================================== */}
