...
export default function KurvaS({ selectedProject }) {
  ...
  // Mengambil state proyek dari router atau props
  ...
  // --- FUNGSI EXPORT MENGIRIM GAMBAR & DATA TABEL KE BACKEND ---
  const executeExport = async () => {
    ...
    try {
      ...
      setTimeout(() => alert(`File ${type.toUpperCase()} Kurva S beserta tabel berhasil diunduh.`), 500);

    } catch (error) {
      console.error(`Gagal export ${type}:`, error);
  ...
  useEffect(() => {
    ...
    // Safety check jika rab_data kosong
    ...
    // Fallback tanggal jika tidak ada
    const startDate = scheduleData.project_info?.tanggal_mulai ? new Date(scheduleData.project_info.tanggal_mulai) : new Date();
    startDate.setHours(0,0,0,0);
    const endDate = scheduleData.project_info?.tanggal_selesai ? new Date(scheduleData.project_info.tanggal_selesai) : new Date(startDate.getTime() + (30 * 24 * 60 * 60 * 1000)); // Default 30 hari
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-20 bg-white dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700/60">
        <Loader2 className="w-10 h-10 text-amber-500 animate-spin mb-3" />
        <p className="font-medium text-slate-600 dark:text-slate-400">Memproses Matrix S-Curve Terkini...</p>
      </div>
    );
  }

  // --- CEK JIKA SCHEDULE KOSONG ---
  ...
  return (
    <div className="w-full space-y-5 pb-20 relative">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 shrink-0 mb-2">
        ...
        <div className="flex items-start lg:items-center gap-3 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* SUB-HEADER DENGAN TAG KATEGORI DINAMIS */}
        ...
        <div className="flex flex-col lg:flex-row items-center gap-2 w-full lg:w-auto mt-2 lg:mt-0">
          ...
          {/* BOX KIRI: FILTER POP-UP & TOMBOL EXPORT */}
          ...
          {/* BOX KANAN: NAVIGASI MENU UTAMA */}
          <div className="flex items-center w-full lg:w-auto justify-between gap-1 bg-white dark:bg-slate-800/80 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700/60 shadow-sm overflow-x-auto hide-scrollbar z-10">
      ...
      {/* --- WARNING BANNER: MUNCUL JIKA TIME SCHEDULE KOSONG --- */}
      {isScheduleEmpty && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 z-10 relative">
        ...
        <div id="chart-area" className="lg:col-span-7 bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 p-5 rounded-2xl flex flex-col shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 dark:border-slate-700/60 pb-3 mb-4 gap-3">
            ...
            {/* Legenda Titik Rencana & Realisasi */}
      ...
      <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className={`overflow-x-auto ${viewMode === 'harian' ? 'max-h-[500px]' : ''}`}>
