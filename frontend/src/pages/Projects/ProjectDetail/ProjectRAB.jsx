...
// PERBAIKAN DI SINI: Menerima props 'currentRabs' agar sesuai dengan ProjectRAB.jsx
export default function FilterRAB({ currentRabs, activeDivisi, setActiveDivisi, searchQuery, setSearchQuery }) {
  ...
  // MENCEGAH CRASH: Jika currentRabs belum siap/kosong, jangan render filternya
  if (!currentRabs || currentRabs.length === 0) return null;
      ...
      <div className="flex flex-col md:flex-row gap-3 justify-between items-start md:items-center w-full transition-all">
        <div className="relative w-full md:flex-1 shadow-sm">
          ...
          <input 
            ...
            className="w-full bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 rounded-xl pl-9 pr-8 py-2.5 text-xs text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition-colors"
          />
          {searchQuery && (
        ...
        <div className="relative w-full md:w-auto" ref={filterRef}>
          <button 
            ...
            className={`w-full md:w-auto flex items-center justify-center gap-2 p-2.5 md:px-4 md:py-2.5 rounded-xl text-xs font-bold border transition-all shadow-sm ${
