...
export default function PetaGIS() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      // Mengubah format edit form menjadi satu kolom
      setEditForm({
  ...
  // --- HANDLER SIMPAN (MENYIMPAN KOORDINAT) ---
  const handleSaveGIS = async (e) => {
    ...
    try {
      ...
      if (projectZones.length > 0) {
        ...
        alert("Data Peta berhasil diperbarui!");
      } else {
        await api.post(`/projects/${id}/gis`, zonePayload);
        alert("Peta Spasial Proyek berhasil diinisialisasi!");
      }
      ...
      console.error(error);
      alert("Gagal menyimpan data spasial. Periksa format koordinat atau koneksi server.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const getBoundsWithPadding = (lat1, lon1, lat2, lon2) => {
    ...
    const minLat = Math.min(l1, l2) - PADDING;
    const maxLat = Math.max(l1, l2) + PADDING;
    const minLon = Math.min(ln1, ln2) - PADDING;
    const maxLon = Math.max(ln1, ln2) + PADDING;
  ...
  const triggerFocusMain = (coordAwalStr, coordAkhirStr) => {
    const coordAwal = parseCoord(coordAwalStr);
    const coordAkhir = parseCoord(coordAkhirStr);
    if (!coordAwal || !coordAkhir) return;
    setFocusBounds(getBoundsWithPadding(coordAwal[0], coordAwal[1], coordAkhir[0], coordAkhir[1]));
  };

  const triggerFocusReport = (latA, lonA, latB, lonB) => {
    if (!latA || !lonA || !latB || !lonB) return;
    const polygonCoords = createRotatedBox(latA, lonA, latB, lonB);
    setFocusBounds(getBoundsFromPolygon(polygonCoords));
  };
  ...
  reportZones.forEach((z, idx) => {
    if (!hiddenReports.has(idx)) { 
      const rBox = createRotatedBox(z.latA, z.lonA, z.latB, z.lonB);
      allBounds.push(...getBoundsFromPolygon(rBox));
    }
  });
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] w-full">
      ...
      <p className="text-slate-500 font-medium">Memuat Peta & Data Spasial...</p>
    </div>
  );

  if (errorMsg || !projectData) return (
    ...
    <div className="w-full space-y-5 pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 shrink-0 mb-2">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-2 w-full lg:w-auto mt-2 lg:mt-0">
          ...
          {/* TOMBOL EDIT HANYA TAMPIL JIKA PUNYA HAK AKSES */}
      ...
      {/* --- KONDISIONAL RENDER: JIKA DATA GIS KOSONG --- */}
      {isGisEmpty ? (
        <div className="flex flex-col items-center justify-center bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-8 md:p-12 shadow-sm text-center min-h-[50vh]">
          ...
          <div className={`lg:col-span-8 bg-slate-100 dark:bg-slate-900 border rounded-2xl overflow-hidden flex flex-col shadow-sm relative min-h-[500px] lg:min-h-[700px] transition-all z-0 ${isEditMode ? 'border-blue-400/60 dark:border-blue-500/50 ring-2 ring-blue-500/10' : 'border-slate-200 dark:border-slate-700/60'}`}>
            ...
            <div className="flex-1 w-full h-full relative z-0 pt-[45px]">
              ...
              >
                ...
                {/* --- RENDER BOX & GARIS KUNING (AREA PROYEK) --- */}
                {!hideMainProject && hasProjectBounds && (
                  <>
                    <Rectangle 
                      bounds={getBoundsWithPadding(coordAwal[0], coordAwal[1], coordAkhir[0], coordAkhir[1])} 
                      pathOptions={{ color: '#f59e0b', weight: 2, dashArray: '5, 5', fillColor: '#fcd34d', fillOpacity: 0.15 }}
                    >
                    ...
                    <Polyline 
                      positions={[
                        [coordAwal[0], coordAwal[1]],
                        [coordAkhir[0], coordAkhir[1]]
                      ]}
                      pathOptions={{ color: '#f59e0b', weight: 2.5, dashArray: '4, 6' }}
                    />

                    <Marker position={[coordAwal[0], coordAwal[1]]} icon={projectIcon}>
                      <Tooltip direction="top" className="font-sans font-bold text-xs">Titik Awal Proyek</Tooltip>
                    </Marker>
                    <Marker position={[coordAkhir[0], coordAkhir[1]]} icon={projectIcon}>
                      <Tooltip direction="top" className="font-sans font-bold text-xs">Titik Akhir Proyek</Tooltip>
                    </Marker>
                  </>
                )}

                {/* --- RENDER BOX & PIN HIJAU (LAPORAN HARIAN) --- */}
                {reportZones.map((rep, idx) => {
                  if (hiddenReports.has(idx)) return null; 
                  return (
                    <React.Fragment key={`rep-${idx}`}>
                      <Polygon 
                        positions={createRotatedBox(rep.latA, rep.lonA, rep.latB, rep.lonB)} 
                        pathOptions={{ color: '#10b981', weight: 2, fillColor: '#34d399', fillOpacity: 0.5 }}
                      >
                      ...
                      <Marker position={[rep.latA, rep.lonA]} icon={reportIcon}>
                        <Tooltip direction="top" className="font-sans font-bold text-xs">Titik Awal Laporan: <br/>{rep.uraian}</Tooltip>
                      </Marker>
                      <Marker position={[rep.latB, rep.lonB]} icon={reportIcon}>
                        <Tooltip direction="top" className="font-sans font-bold text-xs">Titik Akhir Laporan: <br/>{rep.uraian}</Tooltip>
                      </Marker>
                    </React.Fragment>
                  )
                })}
          ...
          <div className="lg:col-span-4 flex flex-col gap-4">
            ...
            <div className={`bg-white dark:bg-slate-800/60 border rounded-2xl p-4 md:p-5 space-y-4 shadow-sm transition-all ${isEditMode ? 'border-blue-400/60 dark:border-blue-500/50 ring-2 ring-blue-500/10' : 'border-slate-200 dark:border-slate-700/60'}`}>
              ...
              <div className="space-y-3 text-xs">
                ...
                {/* Wilayah Administrasi (Read Only) */}
                <div className="bg-slate-50 dark:bg-slate-900/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700/50">
                  <span className="text-slate-500 dark:text-slate-400 text-[10px] block mb-1">Wilayah Administrasi</span>
                ...
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-[10px] font-bold text-amber-600 dark:text-amber-500 uppercase tracking-wider flex items-center gap-1.5"><MapIcon className="w-3.5 h-3.5"/> Koordinat Batas Area (Kuning)</h4>
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="bg-amber-50/50 dark:bg-amber-900/10 p-3 rounded-xl border border-amber-200 dark:border-amber-500/30">
                    <span className="text-amber-700 dark:text-amber-500/80 text-[10px] block mb-1">Koordinat Awal</span>
                    {isEditMode ? (<input type="text" name="koordinat_awal" value={editForm.koordinat_awal || ''} onChange={handleChange} placeholder="-3.300, 114.595" className="w-full bg-white dark:bg-slate-900 border border-amber-300 dark:border-amber-500/50 rounded-lg px-2.5 py-1.5 text-slate-800 dark:text-white font-mono text-[11px] text-center" />) : (<span className="text-slate-700 dark:text-slate-300 font-mono font-bold text-[11px] block text-center">{mainZone?.koordinat_awal || '-'}</span>)}
                  </div>
                  <div className="bg-amber-50/50 dark:bg-amber-900/10 p-3 rounded-xl border border-amber-200 dark:border-amber-500/30">
                    <span className="text-amber-700 dark:text-amber-500/80 text-[10px] block mb-1">Koordinat Akhir</span>
                    {isEditMode ? (<input type="text" name="koordinat_akhir" value={editForm.koordinat_akhir || ''} onChange={handleChange} placeholder="-3.301, 114.598" className="w-full bg-white dark:bg-slate-900 border border-amber-300 dark:border-amber-500/50 rounded-lg px-2.5 py-1.5 text-slate-800 dark:text-white font-mono text-[11px] text-center" />) : (<span className="text-slate-700 dark:text-slate-300 font-mono font-bold text-[11px] block text-center">{mainZone?.koordinat_akhir || '-'}</span>)}
                  </div>
                </div>

                {isEditMode && (
                  <div className="bg-slate-50 dark:bg-slate-900/60 p-3 rounded-xl border border-blue-200 dark:border-blue-900/50 space-y-2 mt-4">
                    <span className="text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase">Data Poligon (GeoJSON)</span>
                    <textarea name="geojson_data" rows="3" value={editForm.geojson_data || ''} onChange={handleChange} className="w-full bg-white dark:bg-slate-900 border border-blue-300 dark:border-blue-500/50 rounded-lg px-2.5 py-1.5 text-[10px] font-mono text-slate-800 dark:text-slate-300" />
                  </div>
                )}
              </div>
            </div>

            <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl flex flex-col overflow-hidden shadow-sm flex-1 min-h-[250px]">
              <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-700/60 bg-emerald-50/50 dark:bg-emerald-900/10 flex items-center justify-between">
                <h3 className="text-xs font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-wider flex items-center gap-2"><CheckCircle2 className="w-4 h-4" /> Progres Harian (Hijau)</h3>
              ...
              <div className="p-4 space-y-3 max-h-[350px] overflow-y-auto hide-scrollbar">
                {reportZones.length === 0 ? (
                  ...
                  reportZones.map((rep, idx) => {
                    ...
                    return (
                      <div key={idx} className={`p-3 rounded-xl border-l-2 transition-all ${isHidden ? 'bg-slate-50/50 dark:bg-slate-900/30 border-l-slate-300 dark:border-l-slate-600 opacity-60' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700/50 border-l-emerald-500'}`}>
                        <div className="flex justify-between items-start mb-1">
                          <p className={`text-[9px] font-bold ${isHidden ? 'text-slate-400' : 'text-emerald-600 dark:text-emerald-400'}`}>{rep.tanggal}</p>
                          <div className="flex items-center gap-1">
                            <button onClick={() => triggerFocusReport(rep.latA, rep.lonA, rep.latB, rep.lonB)} className={`p-1.5 rounded-md transition-colors ${isHidden ? 'text-slate-300' : 'text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900/40'}`} disabled={isHidden} title="Highlight Area Laporan"><Crosshair className="w-3.5 h-3.5" /></button>
                            <button onClick={() => toggleReportVisibility(idx)} className={`p-1.5 rounded-md transition-colors ${isHidden ? 'text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800' : 'text-emerald-500 hover:bg-emerald-100 dark:hover:bg-emerald-900/40'}`} title={isHidden ? "Tampilkan di Peta" : "Sembunyikan dari Peta"}>{isHidden ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}</button>
                          </div>
                        </div>
            ...
            <div className={`bg-white dark:bg-slate-800/60 border rounded-2xl p-4 md:p-5 space-y-3 shadow-sm flex flex-col transition-all ${isEditMode ? 'border-blue-400/60 dark:border-blue-500/50 ring-2 ring-blue-500/10' : 'border-slate-200 dark:border-slate-700/60'}`}>
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700/60 pb-3">
                ...
                {/* TOMBOL UPLOAD DOKUMEN HANYA UNTUK ROLE TERTENTU DI MODE EDIT */}
                {isEditMode && canCreateData && (
                  <>
                    ...
                    <button onClick={() => document.getElementById('docUploadGIS').click()} className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-medium rounded-lg border border-emerald-200 dark:border-emerald-500/20"><UploadCloud className="w-3.5 h-3.5" /> Upload File</button>
                  </>
                )}
              </div>
              <div className="flex-1 space-y-3 text-xs flex flex-col">
                <div className="flex items-start gap-3 p-3 bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-200 dark:border-emerald-500/20 rounded-xl shrink-0">
                  <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                  <div className="w-full">
                    <p className="font-semibold text-emerald-600 dark:text-emerald-400 mb-1">Status Pembebasan Lahan</p>
                    {isEditMode ? (<textarea name="status_lahan" rows="2" value={editForm.status_lahan || ''} onChange={handleChange} className="w-full bg-white dark:bg-slate-900 border border-emerald-300 dark:border-emerald-500/50 rounded-lg px-2.5 py-1.5 text-slate-800 dark:text-white resize-none mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500" />) : (<p className="text-slate-600 dark:text-slate-300 leading-relaxed">{mainZone?.status_lahan || 'Belum ada catatan status lahan.'}</p>)}
                  </div>
                </div>
                <div className="space-y-2 flex-1 overflow-y-auto max-h-[150px] pr-1">
                  {gisDocuments.length === 0 ? (
                     <div className="text-center p-4 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl"><p className="text-slate-500 font-medium">Belum ada dokumen GIS</p></div>
                  ) : (
                    gisDocuments.map((doc) => (
                      <div key={doc.id} className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700/50 rounded-xl group transition-all">
                        ...
                        <div className="flex gap-2">
                          <button type="button" onClick={() => window.open(getDocUrl(doc.path_file), '_blank')} className="p-1.5 bg-white dark:bg-slate-800 text-amber-500 rounded shadow-sm hover:bg-amber-50 dark:hover:bg-slate-700 transition-colors"><Download className="w-3.5 h-3.5"/></button>
                          {/* TOMBOL DELETE DOKUMEN */}
                          {isEditMode && canCreateData && (<button type="button" onClick={() => confirmDeleteDocument(doc.id, doc.nama_file)} className="p-1.5 bg-white dark:bg-slate-800 text-rose-500 rounded shadow-sm hover:bg-rose-50 dark:hover:bg-slate-700 transition-colors"><Trash2 className="w-3.5 h-3.5"/></button>)}
                        </div>
                      </div>
                    ))
                  )}
            ...
            <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-4 md:p-5 shadow-sm">
              <a href={`https://www.google.com/maps/search/?api=1&query=${mainZone?.koordinat_awal || '-1.5,115.0'}`} target="_blank" rel="noopener noreferrer" className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-white dark:text-slate-950 font-bold text-xs rounded-xl flex items-center justify-center gap-2 shadow-md transition-colors"><Navigation className="w-4 h-4" /> Buka Navigasi di Google Maps</a>
            </div>
          </div>
        </div>
      )}
